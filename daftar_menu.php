							...
							<h4>Masakan Hari Ini</h4>
                                <table class="table">
                                    ...
                                    <tbody>
										...
										foreach ($hari as $h)
										{
										$sql = " select * from menu where jenis = 'Sarapan'";
										$hasil = mysqli_query($konek, $sql);
										$data = mysqli_fetch_assoc($hasil);

										$sql = " select * from menu where jenis = 'Makan Siang'";
										$hasil = mysqli_query($konek, $sql);
										$data2 = mysqli_fetch_assoc($hasil);
										
										$sql = " select * from menu where jenis = 'Makan Malam'";
										$hasil = mysqli_query($konek, $sql);
										$data3 = mysqli_fetch_assoc($hasil);
										echo "
																						<tr>
													<td>$h</td>
													<td><input type='checkbox'name='nama_menu[]' value='$data[id_menu]'>$data[nama_paket]</input>
													
													<label data-toggle='modal' data-target='#myModal_sarapan_$h'>Detail</label>
													</td>
													
													
													<td><input type='checkbox'name='nama_menu[]' value='$data2[id_menu]'>$data2[nama_paket]</input>
													
													<label data-toggle='modal' data-target='#myModal_makan_siang_$h'>Detail</label>
													</td>
													
													
													<td><input type='checkbox'name='nama_menu[]' value='$data3[id_menu]'>$data3[nama_paket]</input>
